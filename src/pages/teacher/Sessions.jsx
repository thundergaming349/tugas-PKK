...
export default function TeacherSessions() {
  ...
  const handleCreateSession = async (e) => {
    ...
    try {
      ...
      setFormData({ class_id: '', subject_id: '', date: '', start: '', end: '' });
      fetchData();
    } catch (err) {
      setMessage({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Sesi yang sedang aktif / belum selesai</p>
        </div>
        <button
      ...
      {sessions.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-gray-400 text-sm mt-1">
            Semua sesi telah selesai. Cek halaman <Link to="/teacher/archive" className="text-accent hover:underline">Arsip</Link> atau buat sesi baru.
          </p>
        </div>
      ) : (
      ...
      >
        <form onSubmit={handleCreateSession} className="space-y-4">
          <div>
            ...
            <select name="class_id" value={formData.class_id} onChange={handleChange} className="input-field" required>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="time" name="start" value={formData.start} onChange={handleChange} className="input-field" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Waktu Selesai</label>
              <input type="time" name="end" value={formData.end} onChange={handleChange} className="input-field" required />
            </div>
          </div>
          <div className="flex gap-3 pt-4">
            ...
            <button type="submit" disabled={creating} className="btn-primary flex-1 flex items-center justify-center gap-2">
              {creating ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>Membuat...</>) : ('Buat Sesi')}
            </button>
          </div>
        </form>
      </Modal>
