...
export default function UserManagement() {
    ...
    const handleDelete = async (id) => {
        if (
            !confirm(
                "Yakin ingin menghapus pengguna ini? Semua data terkait (termasuk absensi) akan ikut terhapus.",
            )
        )
            return;
        try {
    ...
    const getRoleBadge = (role) => {
        const maps = {
            admin: "px-2.5 py-1 text-xs font-semibold rounded-full bg-purple-50 text-red-600 border border-red-100 uppercase tracking-wider",
        ...
        return (
            maps[role] ||
            "px-2.5 py-1 text-xs font-semibold rounded-full bg-gray-50 text-gray-600 border border-gray-100 uppercase tracking-wider"
        );
    };
